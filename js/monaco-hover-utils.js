const hoverUtils = {
  ...
  descRegexps: [
    ...
    [/^(get|patch|put|post|delete|head|options|trace|connect):/, 'The OPTIONAL properties get, patch, put, post, delete, head, and options of a resource define its methods; these correspond to the HTTP methods defined in the HTTP version 1.1 specification RFC2616 and its extension, RFC5789.'],
    [/^(get|patch|put|post|delete|head|options|trace|connect)\?:/, 'A resource type definition MAY append a question mark (?) suffix to the name of any method to declare the method as optional.'],
  ...
  parseUrl: function (lineContent) {
    ...
    if (!url.startsWith('http')) {
      const base = (
        (window.appModel && window.appModel.loadedRamlUrl) ||
